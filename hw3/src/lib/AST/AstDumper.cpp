...
void AstDumper::visit(FunctionNode& p_function) {
    ...
    for(DeclNode* decl : p_function.getDeclVec()){
    ...
    std::printf("function declaration <line: %u, col: %u> %s %s %s\n",
            ...
            p_function.getFunctName(), p_function.getReturnType(), proto_type);
...
void AstDumper::visit(VariableNode& p_variable) {
    ...
    std::printf("variable <line: %u, col: %u> %s %s\n",
                ...
                p_variable.getVarName(), p_variable.getVarType());
...
void AstDumper::visit(ConstantValueNode& p_constant_value) {
    ...
    // std::printf("%s\n", p_constant_value.getType());
    if(strcmp(p_constant_value.getType(), "integer") == 0){
        std::printf("constant <line: %u, col: %u> %d\n",
                    p_constant_value.getLocation().line,
                    p_constant_value.getLocation().col,
                    p_constant_value.getValue().int_value);
    }else if(strcmp(p_constant_value.getType(),"real") == 0){
        std::printf("constant <line: %u, col: %u> %lf\n",
                    p_constant_value.getLocation().line,
                    p_constant_value.getLocation().col,
                    p_constant_value.getValue().real_value);
    }else if(strcmp(p_constant_value.getType(), "boolean") == 0){
        std::printf("constant <line: %u, col: %u> %s\n",
                    p_constant_value.getLocation().line,
                    p_constant_value.getLocation().col,
                    p_constant_value.getValue().str_value);
    }else if(strcmp(p_constant_value.getType(), "string") == 0){
        std::printf("constant <line: %u, col: %u> %s\n",
                    p_constant_value.getLocation().line,
                    p_constant_value.getLocation().col,
                    p_constant_value.getValue().str_value);
